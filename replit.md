...
- June 25, 2025. Migrated blog system from Vercel/Neon to Replit PostgreSQL: created proper database schema with blog_posts, admin_users, and admin_sessions tables in Replit's built-in PostgreSQL, configured admin authentication with bcrypt hashing (admin/pravda2025), populated database with sample blog posts, established complete local development workflow - Admin CRM at /admin-pravda now works with Replit database instead of external Neon dependency, eliminating production deployment complexity and enabling full local control
