...
- June 27, 2025. Enhanced calculators CTA section with scarcity messaging: added animated urgency badge showing "3 места остават за следващото тримесечие" with pulsing red background, highlighted number "3" in white/red contrast, dual CTA buttons (primary "Получете безплатен SEO одит" and secondary "Научете повече за услугата"), enhanced heading with highlighted "трансформация" word for increased engagement and conversion
- June 27, 2025. Fixed calculator implementation by creating service-specific calculators: created calculators-fixed.tsx with unique calculators for each service (SEO Struktor™: traffic/ranking/leads, Trendlab™: followers/views/authority, Clickstarter™: orders/CPC/ROAS, Clientomat™: repeat orders/LTV), updated App.tsx to use new file, eliminated generic "SEO" metrics appearing everywhere, each calculator now shows proper service-specific parameters and results
