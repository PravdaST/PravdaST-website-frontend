...
- June 25, 2025. Successfully deployed complete admin CRM system to production: new Vercel-integrated Neon database connected, all API endpoints working with ES modules format, admin authentication functional with admin/pravda2025 credentials, dashboard fully operational at /admin-pravda with blog management and contacts functionality; added Bulgarian to Latin transliteration for automatic slug generation from Cyrillic titles; fixed blog post delete and publish/unpublish functionality with correct API endpoints; created public blog API endpoints for displaying published posts on /blog page; migrated static blog posts from blog.tsx to database for unified content management; resolved all database schema issues by removing author_id and category_id foreign key constraints, added category field to all SQL queries, created blog-db.tsx and blog-post-db.tsx pages with full database integration, updated App.tsx routing to use database-driven blog system - complete workflow operational: Admin CRM creates posts → Neon Database stores them → Public Blog Pages display them with categories and tags
