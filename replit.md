...
- June 25, 2025. Fixed admin authentication system: resolved "relation users does not exist" error by updating all API endpoints to use admin_users table, corrected bcrypt password verification to use password_hash field instead of password field, created proper admin user with valid bcrypt hash for pravda2025 password, added debug logging to track authentication flow - admin login system now properly configured for both Replit development and Vercel production environments
