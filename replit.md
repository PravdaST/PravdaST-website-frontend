...
- June 25, 2025. Converted blog.tsx from static data to dynamic API loading: removed all 6 hardcoded blog posts, implemented useQuery hook to fetch from /api/blog/posts endpoint, added proper loading states and error handling, transformed API data structure to match component expectations, ensured all posts now load from admin CRM system via database - blog page now fully connected to backend data source
