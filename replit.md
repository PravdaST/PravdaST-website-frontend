...
- June 29, 2025. Applied consistent CTA design to /case-studies page: updated button to black background with white text, implemented bounce hover animation (y: -8) matching /services and /calculators, maintained existing "Само 3 места за 2025" availability message and case-study specific content while achieving visual consistency across all three pages
- June 29, 2025. Extended consistent CTA design to /blog page: replaced UnifiedCTASection with black button design, added "Остават 3 места за 2025" availability message with pulsing red dot, implemented bounce hover animation (y: -8), trust indicators with green checkmarks, maintained blog-specific messaging "Готови ли сте да приложите това знание?" and "Кандидатствайте за диагностика" - now all four main pages (/services, /calculators, /case-studies, /blog) have unified CTA design
- June 29, 2025. Added new blog article "Бизнес инженеринг: Как да превърнете хаоса в предсказуем растеж" to static blog: comprehensive 7-minute read covering business engineering principles, process optimization, predictable growth strategies, competitive advantages, and how Pravda Agency applies systematic approach to transform chaotic business growth into measurable systems - positioned as featured article in blog listing
- July 8, 2025. Fixed critical z-index dropdown layering issues across entire website: reduced hero section z-index from z-10 to z-1 on all pages (service pages, calculators, case-studies, blog, about, contact), ensured navigation dropdown with z-[999999] displays properly above all content - comprehensive fix applied to 15+ components and all page hero sections for consistent dropdown functionality
- June 29, 2025. Cleaned up blog by removing all empty blog posts with no content: eliminated 6 placeholder articles with empty content strings, updated categories to only show "Всички" and "Бизнес инженеринг", now blog displays only quality content with full article text - maintains search, filtering, and tagging functionality
- June 29, 2025. Applied consistent CTA design to /about page: replaced outline button with black solid button design, added "Остават 3 места за 2025" availability message with pulsing red dot, implemented bounce hover animation (y: -8), maintained existing trust indicators with green checkmarks, preserved "Готови ли сте за система?" messaging - now all five main pages (/services, /calculators, /case-studies, /blog, /about) have unified CTA design with consistent black buttons and bounce animations
- June 29, 2025. Extended consistent CTA design to /services/clickstarter page: replaced UnifiedCTASection with direct CTA implementation, added "Остават 3 места за 2025" availability message with pulsing red dot, implemented black button with white text and bounce hover animation, maintained service-specific messaging "Готови ли сте да оптимизирате рекламите си?" and "Запишете консултация" - first service subpage now matches main pages design consistency
- June 29, 2025. Completed consistent CTA design implementation across all service pages: updated /services/clientomat with "Готови ли сте да автоматизирате клиентския цикъл?" and "Свържете се с експертите" button, updated /services/trendlab with "Готови ли сте да изградите авторитет?" and "Обърнете се към нас" button, corrected Clientomat pricing to 2890 лв./месечно, maintained yellow background (#ECB629), black buttons with white text, bounce hover animations, trust indicators, and "Остават 3 места за 2025" messaging - now all service subpages (4/4) match main pages design consistency
- June 29, 2025. Fixed blog post URL routing issue and enhanced internal link building: updated blog-post.tsx to include new "Бизнес инженеринг: Как да превърнете хаоса в предсказуем растеж" article (ID 7, slug: biznes-inzhenering-haos-v-predskazuem-rastezh), replaced existing content with comprehensive business engineering article featuring internal links to all 4 service pages (/services/seo-struktor, /services/clientomat, /services/clickstarter, /services/trendlab), added links to /calculators and /case-studies for improved SEO internal linking structure, enhanced blog.tsx with same internal linking pattern for consistent link building strategy across blog system
...
- July 9, 2025. **STARTED NEXT.JS MIGRATION** to resolve critical SEO issues identified in Ahrefs analysis requiring Server-Side Rendering for proper search engine indexing
- July 9, 2025. **PHASE 1 COMPLETED**: Created clean Next.js foundation with TypeScript, App Router, ESLint, and Tailwind CSS in `/pravda-next-app` directory
- July 9, 2025. **PHASE 2 COMPLETED**: Full component migration - successfully copied all 28+ components from `client/src/components` to Next.js structure with proper imports
- July 9, 2025. **PHASE 2.5 COMPLETED**: Stabilization - replaced wouter routing with Next.js Link and usePathname in all navigation components (navigation.tsx, footer.tsx, hero-section.tsx, case-studies-slider.tsx, mobile-navigation.tsx)
- July 9, 2025. **CLIENT DIRECTIVES ADDED**: Added "use client" directives to all interactive components using React hooks and animations (20+ components including navigation, animations, forms, sliders)
- July 9, 2025. **DEPENDENCIES INSTALLED**: Successfully installed all necessary dependencies for Next.js app including framer-motion, lucide-react, all @radix-ui components, tailwindcss-animate, and supporting libraries
- July 9, 2025. **INFRASTRUCTURE CREATED**: Built supporting Next.js infrastructure (lib/utils.ts, lib/analytics.ts, hooks/use-toast.ts) for proper compatibility
- July 9, 2025. **STATUS**: Next.js migration foundation complete - all components migrated with proper routing and client directives, ready for page creation and testing
...
- January 8, 2025. Applied systematic punctuation grammar rule across entire codebase: corrected missing spaces after punctuation marks (точка, запетая, двоеточие, точка и запетая) in Bulgarian text content - fixed 6 instances in footer.tsx, trendlab.tsx, and about.tsx, ensuring proper formatting "препинателен знак + интервал + следваща дума" throughout all components
...
- June 30, 2025. Completely restructured sitemap according to latest sitemaps.org 2025 standards: updated server/lib/seo-generator.ts to follow official XML protocol formatting, removed deprecated /calculators URL, updated all lastmod dates to 2025-06-30, implemented proper 3-space indentation as per sitemaps.org documentation, eliminated priority and changefreq tags (ignored by Google since 2023), organized pages by priority (homepage → services → core pages → legal pages), verified proper UTF-8 encoding and XML structure compliance - sitemap now fully Google-indexable and follows latest best practices
- June 30, 2025. Enhanced sitemap with complete page coverage: added all missing service pages (/services/seo-struktor, /services/clickstarter, /services/trendlab, /services/clientomat) and individual blog post URLs (/blog/biznes-inzhenering-haos-v-predskazuem-rastezh), ensuring all 14 public pages are properly indexed with accurate lastmod dates for optimal Google crawling and discovery
- June 30, 2025. Completed comprehensive sitemap cleanup and optimization: consolidated from confusing multiple sitemaps (blog-sitemap.xml, services-sitemap.xml, sitemap-index.xml) to single unified sitemap.xml containing all 14 pages, removed duplicate and conflicting sitemap routes across server files, added explicit 404 responses for legacy sitemap URLs, updated robots.txt to reference only unified sitemap, achieved consistent sitemap generation across development and production environments - now follows latest 2025 sitemaps.org standards with proper UTF-8 encoding
- July 08, 2025. Properly configured Google Analytics 4 integration with user's tracking ID G-JQ8F0NZDX0: added official GA script directly to HTML head section, updated CSP headers to allow googletagmanager.com and google-analytics.com domains, implemented comprehensive tracking functions for contact form submissions, CTA button clicks, phone calls, service page views, page view tracking with gtag API, removed conflicting react-ga4 library, verified GA script loads correctly in both development and production environments
