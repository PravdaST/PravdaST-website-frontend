      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-foreground mb-6">
            Вижте как работи <span className="text-accent">QR менюто</span>
          </h2>
        ...
        <div className="flex flex-wrap justify-center gap-4 mb-12">
          {categories?.map((category) => (
            <button
              ...
              className={`flex items-center space-x-3 px-6 py-4 rounded-2xl font-semibold transition-all duration-300 conversion-shadow ${
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-start">
          ...
          <div className="order-2 lg:order-1">
            <div className="bg-gradient-to-br from-muted to-muted/50 rounded-3xl p-8 conversion-shadow">
              ...
              <div className="bg-white rounded-2xl p-8 mb-6 flex items-center justify-center relative">
                <div className={`w-48 h-48 bg-gray-900 rounded-xl flex items-center justify-center transition-all duration-500 ${
                  isScanning ? 'scale-110 ring-4 ring-accent ring-opacity-50' : ''
                }`}>
                ...
                {isScanning && (
                  <div className="absolute inset-0 flex items-center justify-center">
                    <div className="w-56 h-56 border-4 border-accent rounded-xl animate-ping"></div>
                  </div>
                )}
              </div>
              ...
              <Button
                ...
                fullWidth
                size="lg"
                className={`${currentCategory?.color} hover:opacity-90 text-white font-semibold`}
                iconName="Scan"
                iconPosition="left"
                loading={isScanning}
              >
              ...
              <div className="grid grid-cols-3 gap-4 mt-8">
                <div className="text-center">
                  <div className="text-2xl font-bold text-accent">2.3с</div>
                  <div className="text-sm text-muted-foreground">време за сканиране</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl font-bold text-success">100%</div>
                  <div className="text-sm text-muted-foreground">мобилна оптимизация</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl font-bold text-primary">24/7</div>
                  <div className="text-sm text-muted-foreground">достъпност</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="order-1 lg:order-2">
            <div className="max-w-sm mx-auto">
              <div className="bg-gray-900 rounded-3xl p-4 conversion-shadow">
                      ...
                      <div
                        ...
                        className={`flex items-center space-x-4 p-3 rounded-xl cursor-pointer transition-all duration-300 ${
                          selectedItem?.id === item?.id
                            ? 'bg-accent/10 ring-2 ring-accent' :'hover:bg-muted/50'
                        }`}
                      >
                        ...
                        <div className="text-right">
                          ...
                          {selectedItem?.id === item?.id && (
                            <div className="text-xs text-success">Избрано ✓</div>
                          )}
                        </div>
                      </div>
                    ))}
        ...
        {/* Demo Results */}
        {selectedItem && (
          <div className="mt-12 bg-gradient-to-r from-success/10 to-accent/10 rounded-2xl p-8 text-center">
            <div className="flex items-center justify-center space-x-2 mb-4">
              <Icon name="CheckCircle" size={24} className="text-success" />
              <h3 className="text-xl font-bold text-foreground">Поръчката е получена!</h3>
            </div>
            <p className="text-muted-foreground mb-4">
              Клиентът избра: <span className="font-semibold text-accent">{selectedItem?.name}</span> за <span className="font-semibold text-accent">{selectedItem?.price}</span>
            </p>
